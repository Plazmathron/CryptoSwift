...
public struct CTRXOR: StreamMode {
...
struct CTRXORModeWorker: StreamModeWorker, SeekableModeWorker, CounterModeWorker {
    typealias Counter = CTRCounter
    
    final class CTRCounter {
        private var initialValue: Array<UInt8>
        private let constPrefix: Array<UInt8>
        private var value: UInt64 = 0
        //TODO: make it an updatable value, computing is too slow
        var bytes: Array<UInt8> {
            return applyXor(initialValue, value)
        }
        ...
        private func applyXor(_ left: Array<UInt8>, _ right: UInt64) -> Array<UInt8>  {
            let noncePartLen = left.count / 2
            let noncePrefix = left[left.startIndex..<left.startIndex.advanced(by: noncePartLen)]
            let nonceSuffix = left[left.startIndex.advanced(by: noncePartLen)..<left.startIndex.advanced(by: left.count)]
            let c = UInt64(bytes: nonceSuffix) ^ right
